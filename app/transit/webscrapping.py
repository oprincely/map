...
def create_eph(n,yr,p):#0,1
    ...
    #{{ url_for('static', filename='ephemeris/{yr}{p}/eph_{yr}{p}_{month_to_num[n]}.txt') }}

    def write_json(data, filename=f'./ephemeris/{yr}{p}/eph_{yr}{p}_{month_to_num[n]}.txt'):
        '''
        if not os.path.exists(f'ephemeris/{yr}{p}/eph_{yr}{p}_{month_to_num[n]}.txt'):
            os.mkdir(f'ephemeris/{yr}{p}/eph_{yr}{p}_{month_to_num[n]}.txt')
        '''
        try:
