...
def astro_cal_tees(btd,btm,bty,transit_day,transit_month,transit_year,asc,mc,prog_moon):
    ...
    arc = solar_arc(btd, month(number, month_to_num[btm-1], bty),[13,9],[13,6], transit_year)
    progressed = [[0, 45], prog_moon, [0, 45], [0, 45], [0, 45]]
    ...
    def cal_tees():
        ...
        def in_transit_move(u, u1, transit, aspect,planet_position,tipe):
            ...
            if u in transit:
                ...
                ns_sym = f'{tipe} {to_symbol(f2)} {aspect_to_symbol(aspect)} {to_symbol(which_natal[1])}' #♃_☉/☽
        ...
        def rip_transit_movemont(conj): #in_transit_move(u,u1, transit, aspect,planet_position,str(tipe))
            all_current_transit = []
            #all_current_tmpt = []
            def what_to_natal():
                current_transitn = []
                ...
                while planet_position != len(naspect1):
                    ...
                    current_transitn.append(in_transit_move(u,u1, arc, aspect,planet_position,'arc'))
                    current_transitn.append(in_transit_move(u,u1, progressed, aspect,planet_position,'Pr'))
            ...
            def what_to_midpts():
                ...
                while n != len(midpts):
                    ...
                    current_transitm.append(in_transit_move_midpts(u,u1, arc, aspect,mname,symb,'arc'))
                    current_transitm.append(in_transit_move_midpts(u,u1, progressed, aspect,mname,symb,'Pr'))
        ...
        def do_calculate_for_natal():
            ...
            sextile_trine.append(rip_transit_movemont(trine))
            sextile_trine.append(rip_transit_movemont(sextile))
            
            def clean_up(what_to_clean,where_to_append_cleaned):
                for item in what_to_clean:
                    if item != '':
                        where_to_append_cleaned.append(item)
                    else:
                        pass
                return where_to_append_cleaned
            
            clean_up(to_natals,to_natals_clean)
            clean_up(sextile_trine,sextile_trine_clean)
           
            return to_natals_clean,sextile_trine_clean
