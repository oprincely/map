...
def users():
    ...
    next_url = url_for('users.users', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('users.users', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('users/users.html', title='Home', form=form,
...
def explore():
    ...
    prev_url = url_for('users.explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('users/users.html', title='Explore', posts=posts.items,
...
@login_required
def messages():
    current_user.last_message_read_time = x
